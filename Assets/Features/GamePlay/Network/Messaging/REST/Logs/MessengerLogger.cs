...
namespace GamePlay.Network.Messaging.REST.Logs
{
    public class MessengerLogger
    {
        ...
        public void OnDirectRequestSent<TRequest>(RagonPlayer player)
        {
            ...
            _logger.Log($"On direct request sent to player: {player.Id} with type: {typeof(TRequest)}", _settings.LogParameters);
        }
        ...
        public void OnDirectRequestReceived<TRequest>(RagonPlayer player)
        {
            ...
            _logger.Log($"On direct request received on player: {player.Id} with type: {typeof(TRequest)}", _settings.LogParameters);
        }
        ...
        public void OnDirectResponseSent<TResponse>(RagonPlayer player)
        {
            ...
            _logger.Log($"On direct response sent to player: {player.Id} with type: {typeof(TResponse)}", _settings.LogParameters);
        }
        ...
        public void OnDirectResponseReceived<TResponse>(RagonPlayer player)
        {
            ...
            _logger.Log($"On direct response received on player: {player.Id} with type: {typeof(TResponse)}", _settings.LogParameters);
        }
        ...
        public void NoResponseHandlerFoundException<TResponse>(Guid requestId)
        {
            ...
            _logger.Log($"No response handler of type: {typeof(TResponse)} with id: {requestId}", _settings.LogParameters);
        }
        ...
        public void OnOwnerRequestSent<TResponse>(NetworkPlayer player)
        {
            ...
            _logger.Log($"On response sent to owner: {player.Id}/{player.DisplayName} with type: {typeof(TResponse)}", _settings.LogParameters);
        }
